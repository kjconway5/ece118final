...
ES_Event RunInISZSubHSM(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        ...
        case Charge1:
            ...
            if ((ThisEvent.EventType == ES_TIMEOUT) && (ThisEvent.EventParam == SPINUP_TIMER)) {
                nextState = ShakeForward;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
            break;
case ShakeForward:
            if (ThisEvent.EventType == ES_ENTRY) {
                DriveForward(1000);
                ES_Timer_InitTimer(SHAKE_TIMER, SHAKE_TIME);
            }
            ...
            if ((ThisEvent.EventType == ES_TIMEOUT) && (ThisEvent.EventParam == SHAKE_TIMER)) {
                count++; 
                if (count > 10) {
                    count = 0;
                    nextState = Charge1;
